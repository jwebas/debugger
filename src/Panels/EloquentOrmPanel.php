...
class EloquentOrmPanel extends DebuggerPanel
{
    /**
     * @var string
     */
    ...
    {
        $data = $this->getInfo();
        $title = $this->getIcon() . ' ' . $this->title . ' ' . $data['count'] . ' / ' . $data['time'] . 'ms';

        $content = '<div class="tracy-inner">';
        $content .= $this->tableHeader(['No', 'Time, ms', 'Query / Bindings']);
        $content .= $this->getInfo()['content'];
    ...
    {
        /** @var Manager $manager */
        $manager = $this->container->get('database');
